...
def base_heuristic(_grid_robot_state):
    ...
    manhattan_distance = abs(_grid_robot_state.location[0] - _grid_robot_state.lamp_location[0]) + \
                         abs(_grid_robot_state.location[1] - _grid_robot_state.lamp_location[1])

    # Tie-breaking bias: prioritize higher rows (smaller x-values) and leftward columns (smaller y-values)
